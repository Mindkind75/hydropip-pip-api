...
export function fallbackAnswer(question = "", retrieval = { matches: [] }) {
  ...
  if (/\b(shorter|short|smaller|fit|space|footprint|height|compact|scale down|two towers|2 towers|fewer towers|less towers)\b/.test(q) && /\b(tower|towers|system|pots|pot)\b/.test(q)) {
    return `${contextLead}Yes, you can scale HydroPip down. Two shorter towers are a reasonable small build if they stay stable, have service space around them, and each tower still has a securely driven center support pipe.\n\nOne clarification: HydroPip uses four-pot stackable sections. If you mean five stack sections high, that is 20 planting pockets per tower. Keep the loose 3/4 inch tee as the removable top hose guide and recalibrate feed time from runoff, because shorter towers need less lift and may wet faster. Tell me your available width, depth, and height and I will sanity-check the layout.`;
  }
