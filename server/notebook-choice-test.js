...
await ok('/users/me/preferences','PATCH',{patch:{notebookMode:'custom',workspaceTools:['planner','seeds'],workspaceStartTab:'planner',lastGrowId:p.id}});const prefs=(await ok('/users/me/preferences')).preferences;assert.deepEqual(prefs.workspaceTools,['planner','seeds']);assert.equal(prefs.workspaceStartTab,'planner');
...
if(process.argv.includes('--ui')){
 ...
 await page.locator('#proArrangeTabs').click();await page.locator('#notebookStartTab').selectOption('seeds');await page.locator('#proTabOrderForm button[type=submit]').click();await page.locator('#proTabOrderDialog').waitFor({state:'hidden'});
 ...
 assert.equal(await second.locator('#notebookGuidanceMode').isVisible(),true);
 assert.deepEqual(await second.locator('#notebookGuidanceMode option').allTextContents(),['Guide me','Full notebook','Custom']);
 await second.locator('#notebookGuidanceMode').selectOption('full');await second.waitForFunction(()=>!document.querySelector('#notebookGuidanceMode').disabled);assert.equal(await second.locator('#notebookAllTools').isVisible(),false);assert.equal(await second.locator('[data-pro-page=crops]').isVisible(),true);
 await second.locator('#notebookGuidanceMode').selectOption('guided');await second.waitForFunction(()=>!document.querySelector('#notebookGuidanceMode').disabled);assert.equal(await second.locator('#proFocusPanel').isVisible(),true);assert.equal(await second.locator('#proFocusTools button').count(),1);
 await second.locator('#notebookGuidanceMode').selectOption('custom');await second.waitForFunction(()=>!document.querySelector('#notebookGuidanceMode').disabled);assert.equal(await second.getByRole('checkbox',{name:'Hide My Crops',exact:true}).isChecked(),true);
 for(const box of await second.locator('#notebookHiddenChoices input').all())await box.uncheck();
 await second.getByRole('checkbox',{name:'Hide My Crops',exact:true}).check();await second.locator('#notebookSaveHidden').click();await second.waitForFunction(()=>!document.querySelector('#notebookSaveHidden').disabled);assert.equal(await second.locator('#notebookCustomTools').evaluate(el=>el.open),false);await second.locator('#notebookCustomTools summary').click();assert.equal(await second.getByRole('checkbox',{name:'Hide My Crops',exact:true}).isVisible(),true);assert.equal(await second.getByRole('checkbox',{name:'Hide My Crops',exact:true}).isChecked(),true);await second.locator('#notebookSaveHidden').click();await second.waitForFunction(()=>!document.querySelector('#notebookSaveHidden').disabled);assert.equal(await second.locator('#notebookCustomTools').evaluate(el=>el.open),false);assert.equal(await second.locator('[data-pro-page=crops]').isVisible(),false);
 await second.locator('#notebookAllTools').click();assert.equal(await second.locator('#notebookAllTools').innerText(),'Hide tools');assert.equal(await second.locator('[data-pro-page=crops]').isVisible(),true);await second.locator('#notebookAllTools').click();assert.equal(await second.locator('[data-pro-page=crops]').isVisible(),false);
 for(const width of [390,1280]){await second.setViewportSize({width,height:900});assert.equal(await second.locator('#notebookTabRow .workspace-tabs').evaluate(e=>getComputedStyle(e).display),'flex');const tops=await second.locator('#notebookTabRow .workspace-tabs .workspace-tab:visible').evaluateAll(nodes=>nodes.map(e=>Math.round(e.getBoundingClientRect().top)));assert.equal(new Set(tops).size,1);assert.equal(await second.evaluate(()=>document.documentElement.scrollWidth>innerWidth+2),false);}
 await second.screenshot({path:path.join(out,'guidance-desktop.png'),fullPage:true});await second.setViewportSize({width:390,height:900});await second.screenshot({path:path.join(out,'guidance-mobile.png'),fullPage:true});
 assert.equal((await ok('/users/me/preferences')).preferences.notebookMode,'custom');
 await second.reload();await second.evaluate(d=>window.postMessage(d,location.origin),{type:'HYDROPIP_PIP_SESSION',sessionToken:token,member,subscription});await second.locator('[data-pro-panel=seeds].active').waitFor();await second.waitForFunction(()=>document.querySelector('#proWorkspaceStatus').textContent.includes('memory ready'));assert.equal(await second.locator('#notebookGuidanceMode').inputValue(),'custom');assert.equal(await second.locator('#notebookCustomTools').evaluate(el=>el.open),false);assert.equal(await second.locator('#notebookCustomTools summary').isVisible(),true);assert.equal(await second.locator('[data-pro-page=crops]').isVisible(),false);
 console.log('PASS Three guidance modes, checkbox-to-hide, conditional All tools / Hide tools, one row on phone and desktop, persisted custom settings');
